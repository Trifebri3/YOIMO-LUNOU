...
<body class="font-sans antialiased text-slate-800 min-h-full flex">
    ...
    <div class="fixed bottom-0 inset-x-0 bg-white border-t border-slate-100 z-40 flex items-center justify-around py-2.5 px-4 md:hidden shadow-[0_-4px_24px_rgba(0,0,0,0.06)] rounded-t-2xl">
        <!-- Home / Dashboard -->
        <a href="{{ route('management.dashboard') }}" class="flex flex-col items-center justify-center gap-0.5 {{ Request::routeIs('management.dashboard') ? 'text-indigo-600 font-bold' : 'text-slate-400 hover:text-indigo-600' }} transition-colors">
        ...
        @if($activeCompany)
            <a href="{{ route('management.company.workspace', $activeCompany->id) }}" class="flex flex-col items-center justify-center gap-0.5 {{ Request::routeIs('management.company.workspace') ? 'text-indigo-600 font-bold' : 'text-slate-400 hover:text-indigo-600' }} transition-colors">
