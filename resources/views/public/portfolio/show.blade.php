    ...
    <header class="border-b border-slate-200/80 bg-white/95 sticky top-0 z-40 backdrop-blur-md shadow-xs">
        <div class="max-w-6xl mx-auto px-3 sm:px-6 h-16 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                <a href="{{ url('/') }}" class="flex items-center gap-2 sm:gap-2.5 group">
                    <img src="{{ asset('icon/11.png') }}" alt="Yoimo" class="w-7 h-7 sm:w-8 sm:h-8 rounded-xl object-contain group-hover:scale-105 transition-transform shadow-2xs shrink-0">
                    <div class="leading-tight">
                        <span class="font-black text-base sm:text-lg text-slate-900 tracking-tight block">YOIMO<span class="text-indigo-600 text-xs ml-1 font-bold hidden sm:inline">PORTFOLIO</span></span>
                    </div>
                </a>
                <span class="hidden md:inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200/70 shrink-0">
                    YOTA FAMILY
                </span>
            </div>

            <div class="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
                <button type="button" onclick="shareToLinkedIn()" title="Bagikan ke LinkedIn" class="inline-flex items-center justify-center gap-1.5 px-2.5 sm:px-3.5 py-2 bg-[#0a66c2] hover:bg-[#004182] text-white text-xs font-bold rounded-xl transition shadow-xs shrink-0">
                    <svg class="w-3.5 h-3.5 fill-current shrink-0" viewBox="0 0 24 24"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"/></svg>
                    <span class="hidden sm:inline">Bagikan</span>
                </button>

                <button type="button" onclick="copyPortfolioLink()" title="Salin Tautan" class="inline-flex items-center justify-center gap-1.5 px-2.5 sm:px-3.5 py-2 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-xl transition border border-slate-200 shadow-2xs shrink-0">
                ...
                @auth
                    <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-1 px-2.5 sm:px-3 py-2 text-xs font-bold text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50/80 rounded-xl transition shrink-0" title="Ke Dashboard">
                        <span class="hidden sm:inline">Dashboard</span>
                        <span>&rarr;</span>
                    </a>
                @else
                    <a href="{{ route('login') }}" class="px-2.5 sm:px-3 py-2 text-xs font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition shrink-0">Masuk</a>
                @endauth
            </div>
        </div>
    </header>
