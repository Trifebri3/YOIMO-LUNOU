...
<aside id="mobile-sidebar" class="fixed inset-y-0 left-0 z-50 w-72 sm:w-80 xl:w-64 bg-white border-r border-slate-200/80 flex flex-col justify-between h-[100dvh] max-h-[100dvh] xl:h-screen shrink-0 font-sans hidden xl:flex xl:sticky xl:top-0 shadow-2xl xl:shadow-none overflow-hidden transition-all">
    <div class="p-5 sm:p-6 space-y-6 flex-1 overflow-y-auto min-h-0 overscroll-contain" style="-webkit-overflow-scrolling: touch; touch-action: pan-y; overscroll-behavior-y: contain;">
        ...
        <!-- Tombol Pencarian Cepat Quick Nav -->
        <button type="button" onclick="openQuickNavModal()" 
                class="w-full flex items-center justify-between px-3.5 py-2.5 bg-slate-100/80 hover:bg-indigo-50 border border-slate-200/80 hover:border-indigo-200 rounded-2xl text-xs font-bold text-slate-600 hover:text-indigo-700 transition-all shadow-2xs group">
            <div class="flex items-center gap-2.5">
                <svg class="w-4 h-4 text-slate-400 group-hover:text-indigo-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                <span>Cari Menu / Proyek</span>
            </div>
            <kbd class="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-[9px] font-mono text-slate-500">Ctrl+K</kbd>
        </button>
