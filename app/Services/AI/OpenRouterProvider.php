...
class OpenRouterProvider implements AIProviderInterface
{
    ...
    public function __construct(string $apiKey, string $model, ?string $baseUrl = null)
    {
        $this->apiKey = $apiKey;
        $this->model = $model;
        $this->baseUrl = $baseUrl ?: config('ai.providers.openrouter.base_url', 'https://openrouter.ai/api/v1');
    }
    ...
    public function chat(array $payload): AIResponse
    {
        ...
        try {
            ...
            if ($response->failed()) {
                ...
                $errMsg = $errBody['error']['message'] ?? 'OpenRouter API request failed.';
                Log::error('OpenRouter adapter error: ', ['status' => $status, 'error' => $errBody]);
    ...
    {
        return config('ai.providers.openrouter.models', []);
    }
