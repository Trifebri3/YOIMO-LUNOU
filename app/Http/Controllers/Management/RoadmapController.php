...
class RoadmapController extends Controller
{
    ...
    public function generateRoadmapAi(Project $project): RedirectResponse
    {
        ...
        try {
            ...
            // Catat Audit Log
            ProjectActivityLog::record($project->id, 'Roadmap', 'CREATE', 'Men-generate seluruh alur linimasa proyek secara otomatis menggunakan LUNOU AI.');

            return redirect()->route('management.projects.roadmaps.index', $project->id)
                ->with('success', 'Alur linimasa proyek berhasil di-generate secara otomatis menggunakan LUNOU AI.');

        } catch (\Exception $e) {
            Log::error('Failed to generate Roadmap via AI: '.$e->getMessage());

            return back()->with('error', 'Gagal memproses pembuatan linimasa otomatis: '.$e->getMessage());
        }
    }

    public function suggestObjectivesAi(Request $request, Project $project): JsonResponse
    {
        $request->validate([
    ...
    public function enhanceRoadmapAi(Request $request, Project $project, ProjectRoadmap $roadmap): RedirectResponse
    {
        ...
        try {
            ...
            // Catat Audit Log
            ProjectActivityLog::record($project->id, 'Roadmap', 'UPDATE', "Menyempurnakan fase linimasa [{$roadmap->title}] secara otomatis menggunakan LUNOU AI.");

            return redirect()->route('management.projects.roadmaps.index', $project->id)
                ->with('success', 'Fase linimasa "'.$roadmap->title.'" berhasil disempurnakan menggunakan LUNOU AI.');

        } catch (\Exception $e) {
            Log::error('Failed to enhance Roadmap Phase via AI: '.$e->getMessage());
